---
name: "Sweet Lollapalooza Custom Cakes"
slug: "sweet-lollapalooza"
category: "bakeries"
subcategory: "custom-cakes"
tier: "featured"
description: "Custom celebration cakes, cupcakes, and dessert tables for weddings, birthdays, and corporate events. Halal, egg-free, and gluten-free options available."
address: "Edmonton"
neighborhood: "South Edmonton"
google_maps_url: "https://maps.google.com/?q=Sweet%20Lollapalooza%20Custom%20Cakes%20Edmonton"
hours:
  monday: "9:00 AM - 6:00 PM"
  tuesday: "9:00 AM - 6:00 PM"
  wednesday: "9:00 AM - 6:00 PM"
  thursday: "9:00 AM - 8:00 PM"
  friday: "9:00 AM - 6:00 PM"
  saturday: "9:00 AM - 5:00 PM"
  sunday: "Closed"
photos: ["/photos/sweet-lollapalooza-1.jpg", "/photos/sweet-lollapalooza-2.jpg", "/photos/sweet-lollapalooza-3.jpg"]
rating: 4.8
review_count: 120
price_range: "$$$"
amenities:
  - "Custom Orders"
  - "Delivery"
  - "Halal Options"
  - "Gluten-Free Options"
tags: ["custom-cakes", "wedding-cakes", "cupcakes"]
active: true
date_listed: "2026-08-18"
reviews:
  - name: "Ira Sharma"
    rating: 5
    comment: |
      Someone gifted me this box today, and I am absolutely charmed by these cute little chocolates. I don't know who the person was, but he has a great choice in chocolates and gifting, I must say. Oh yes, and the taste of the chocolates is great too!
  - name: "mystery evolves"
    rating: 5
    comment: |
      Exquisite chocolate boutique experience.
      
      I stepped into this shop to buy a birthday gift for my friend.
      
      The chocolates have elevated flavors and designs, some of them are based on current themes like Valentines day.
      
      The chocolates are all some prettily presented in a box. And, you can get the box wrapped with a ribbon.
      
      Thus was a great shopping experience.
  - name: "L Y"
    rating: 5
    comment: |
      100 % Gluten free! Protip for parking: park in Manulife parkade, it is safe, clean and staffed and reasonably priced for the little bit of time to take to buy chocolate!
  - name: "Lori Bezenar"
    rating: 5
    comment: |
      By far, the best place to purchase chocolates. Either for your own indulgence or as a gift, it's a place worth visiting.
  - name: "Valerie"
    rating: 5
    comment: |
      Delicious chocolates and a lovely experience! The person who helped me was super kind and friendly, made me feel really welcome. Not only were the chocolates amazing, but the whole visit was genuinely heartwarming. 🤩
---
